...
interface PromptHistory {
  id: number;  // Made id required since backend now returns it
  prompt: string;
  ...
  const handleDelete = async (historyId: number) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setError(data.message || 'Failed to delete history entry');
      }
    } catch (err) {
      setError('Error deleting history entry');
    }
  };
  ...
  const handlePrompt = async () => {
    ...
    try {
      ...
      if (predefinedPrompts[lowercasePrompt]) {
        // Store predefined prompt in the backend
        const response = await fetch('http://localhost:5000/generate-sound', {
          ...
          body: JSON.stringify({
            prompt: prompt.trim(),
            model: 'predefined',  // Use a special model name for predefined prompts
            userId,
            audioFilePath: predefinedPrompts[lowercasePrompt],  // Send the predefined audio URL
          }),
