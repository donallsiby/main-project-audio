...
# Remove custom CA bundle configuration
...
device = "cuda" if torch.cuda.is_available() else "cpu"  # Detect GPU
...
def register():
    ...
    try:
        ...
        if e.pgcode == '23505':  # Unique violation (email already exists)
...
def generate_sound():
    try:
        ...
        duration = 10 if model == 'musicgen' else 6 if model == 'audiogen' else None  # None for TTS

        # For caching, use a placeholder for TTS duration since it varies
        ...
        if cache_key in audio_cache:
            ...
            # Still need to store in database for history
            try:
                conn = get_db_connection()
                cur = conn.cursor()
                cur.execute(
